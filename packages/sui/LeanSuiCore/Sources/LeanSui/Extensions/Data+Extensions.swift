...
extension Data {
  ...
  /// Create `Data` from a hex string (optionally `0x`-prefixed). Non-hex input
  /// yields empty `Data`.
  public init(hex: String) {
    self.init(Data.fromHex(hex) ?? Data())
  }
  ...
  /// Computes the SHA-256 hash of the data (cross-platform via swift-crypto).
  /// - Returns: The SHA-256 hash as Data
  public func sha256() -> Data {
    return Data(Crypto.SHA256.hash(data: self))
  }
