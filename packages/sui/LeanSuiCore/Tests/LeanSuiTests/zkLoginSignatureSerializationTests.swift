...
//  LeanSui
//
//  Offline zkLogin signature parse/serialize/round-trip vectors ported from
//  SuiKit (Tests/SuiKitTests/Unit/zkLogin/zkLoginSignatureSerializationTests.swift),
...
final class zkLoginSignatureSerializationTests: XCTestCase {
  ...
  func testParsezkLoginSignature() throws {
    XCTAssertNotNil(Data(base64Encoded: sampleSignature), "Failed to decode base64 signature")

    let parsedSignature = try zkLoginSignature.parse(serialized: sampleSignature)

    XCTAssertEqual(parsedSignature.maxEpoch, 174)
    XCTAssertEqual(
      Data(parsedSignature.userSignature).base64EncodedString(), sampleEphemeralSignature)
    XCTAssertEqual(
      parsedSignature.inputs.addressSeed,
